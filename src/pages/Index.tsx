import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { ArrowRight, Shield, BookOpen, Users, FileText, Handshake } from 'lucide-react';
import { Link } from 'react-router-dom';
import { lazy, Suspense } from 'react';
import Navigation from '@/components/Navigation';
import Footer from '@/components/Footer';

const QuickHelp = lazy(() => import('@/components/QuickHelp'));

const Index = () => {
  const featuredResources = [
    {
      title: "Horse Sale & Loan Contracts",
      description: "Create and understand legally sound agreements.",
      icon: FileText,
      link: "/learn/sale-and-loan-contracts",
      type: "Learning Guide"
    },
    {
      title: "Rider & Instructor Liability",
      description: "Know your responsibilities and protect yourself from risk.",
      icon: Shield,
      link: "/learn/liability",
      type: "Learning Guide"
    },
    {
      title: "Livery Yard Agreements",
      description: "Essential terms for yard owners and horse owners.",
      icon: Handshake,
      link: "/learn/livery-contracts",
      type: "Learning Guide"
    }
  ];

  const quickHelpItems = [
    {
      question: "What are my rights if a horse I bought is lame?",
      answer: "Your rights depend heavily on whether you bought from a business (dealer) or a private seller. The Consumer Rights Act 2015 offers significant protection for buyers in business sales, whereas private sales operate on a 'buyer beware' basis, with recourse mainly through misrepresentation.",
      link: "/learn/returns-and-refunds"
    },
    {
      question: "As a freelance instructor, what insurance do I need?",
      answer: "Freelance instructors in the UK should have, at a minimum, Public Liability insurance and Professional Indemnity insurance. This combination covers you for third-party injury/property damage and for claims arising from the professional advice you give.",
      link: "/learn/instructor-liability"
    },
    {
      question: "My livery yard wants to increase their prices. How much notice should they give?",
      answer: "This should be specified in your livery contract. If not, 'reasonable notice' is expected, which is typically at least one full month. Your contract is the key document defining the terms of your agreement with the yard.",
      link: "/learn/livery-contracts"
    }
  ];

  return (
    <div className="min-h-screen bg-background">
      <Navigation />
      
      {/* Hero Section */}
      <section className="hero-gradient text-white py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl lg:text-6xl font-heading font-bold mb-6">
            Clear Guidance for the
            <span className="block text-equine-accent">UK Equestrian World</span>
          </h1>
          <p className="text-xl text-equine-sage mb-8 leading-relaxed max-w-3xl mx-auto">
            Demystifying equine law for horse owners, riders, and professionals. Free, practical resources on liability, contracts, welfare, and disputes.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/learn">
              <Button size="lg" className="bg-equine-accent text-white hover:bg-equine-forest font-semibold px-8 py-4 text-lg w-full">
                <BookOpen className="mr-2 h-5 w-5" />
                Explore Guides
              </Button>
            </Link>
            <Link to="/tools">
              <Button size="lg" variant="outline" className="border-equine-accent text-equine-accent hover:bg-equine-accent hover:text-white font-semibold px-8 py-4 text-lg w-full">
                Use Interactive Tools
                <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
            </Link>
          </div>
        </div>
      </section>

      {/* Featured Resources */}
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-heading font-bold text-equine-navy mb-4">
              Start Your Journey Here
            </h2>
            <p className="text-xl text-equine-forest max-w-2xl mx-auto">
              Begin with our core guides covering the most common legal questions in the equestrian world.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {featuredResources.map((resource, index) => (
              <Card key={index} className="shadow-lg hover:shadow-xl transition-all duration-300 border-0 bg-white group cursor-pointer">
                <CardHeader className="text-center pb-4">
                  <div className="mx-auto bg-equine-accent p-4 rounded-full mb-4 group-hover:scale-110 transition-transform duration-300">
                    <resource.icon className="h-8 w-8 text-white" />
                  </div>
                  <span className="bg-equine-warm text-equine-forest px-3 py-1 rounded-full text-sm font-semibold mb-2 inline-block">
                    {resource.type}
                  </span>
                  <CardTitle className="text-xl font-heading text-equine-navy">
                    {resource.title}
                  </CardTitle>
                </CardHeader>
                <CardContent className="text-center">
                  <p className="text-equine-forest mb-6">
                    {resource.description}
                  </p>
                  <Link to={resource.link}>
                    <Button className="w-full bg-equine-accent text-white hover:bg-equine-forest font-semibold">
                      Read Guide
                      <ArrowRight className="ml-2 h-4 w-4" />
                    </Button>
                  </Link>
                </CardContent>
              </Card>
            ))}
          </div>
        </div>
      </section>

      {/* Quick Help Section */}
      <section className="py-20 bg-equine-warm">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl font-heading font-bold text-equine-navy mb-4">Quick Legal Questions</h2>
          <p className="text-lg text-equine-forest mb-12">Fast answers to common equestrian legal queries.</p>
        </div>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="space-y-6">
            {quickHelpItems.map((item, index) => (
              <Card key={index} className="shadow-md hover:shadow-lg transition-shadow duration-300">
                <CardHeader>
                  <CardTitle className="text-equine-navy text-lg">{item.question}</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-equine-forest mb-4">{item.answer}</p>
                  <Link to={item.link} className="text-equine-accent font-semibold hover:underline">
                    Learn More <ArrowRight className="inline h-4 w-4" />
                  </Link>
                </CardContent>
              </Card>
            ))}
          </div>
        </div>
      </section>

      {/* Trust & Community */}
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl lg:text-4xl font-heading font-bold text-equine-navy mb-6">
            A Trusted Resource for All Equestrians
          </h2>
          <p className="text-xl text-equine-forest mb-12 max-w-3xl mx-auto">
            Equine Legal Compass was created by legal professionals and equine enthusiasts to provide clear, reliable, and free information for the entire UK equestrian community.
          </p>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="bg-equine-accent p-4 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center">
                <Users className="h-8 w-8 text-white" />
              </div>
              <h3 className="font-heading font-semibold text-equine-navy mb-2">Community Focused</h3>
              <p className="text-equine-forest">Built for riders, owners, and professionals</p>
            </div>
            <div className="text-center">
              <div className="bg-equine-accent p-4 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center">
                <Shield className="h-8 w-8 text-white" />
              </div>
              <h3 className="font-heading font-semibold text-equine-navy mb-2">Always Free & Impartial</h3>
              <p className="text-equine-forest">No hidden costs, no upselling.</p>
            </div>
            <div className="text-center">
              <div className="bg-equine-accent p-4 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center">
                <BookOpen className="h-8 w-8 text-white" />
              </div>
              <h3 className="font-heading font-semibold text-equine-navy mb-2">Practical & Clear</h3>
              <p className="text-equine-forest">Real-world guidance, not dense legal theory</p>
            </div>
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="hero-gradient py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl lg:text-4xl font-heading font-bold text-white mb-6">
            Ready to Navigate Your Equestrian Journey with Confidence?
          </h2>
          <p className="text-xl text-equine-sage mb-8">
            Explore our comprehensive guides or use our interactive tools to get the clarity you need.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/learn">
              <Button size="lg" className="bg-equine-accent text-white hover:bg-equine-forest font-semibold px-8 py-4 text-lg w-full">
                <BookOpen className="mr-2 h-5 w-5" />
                Explore All Guides
              </Button>
            </Link>
            <Link to="/tools">
              <Button size="lg" variant="outline" className="border-equine-accent text-equine-accent hover:bg-equine-accent hover:text-white font-semibold px-8 py-4 text-lg w-full">
                Discover Interactive Tools
              </Button>
            </Link>
          </div>
        </div>
      </section>

      <Footer />
    </div>
  );
};

export default Index;
